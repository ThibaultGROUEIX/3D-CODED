...
class SURREAL(data.Dataset):
    def __init__(self, train, npoints=2500, regular_sampling=False, normal=False, data_augmentation_Z_rotation=False,
                 data_augmentation_Z_rotation_range=360, data_augmentation_3D_rotation=False, cache=True,
                 path="./data/data-surreal/dataset_centered/", path_2 = "./data/data-surreal/dataset_augmented/", path_val = "./data/data-surreal/dataset-gaussian-val/"):
            ...
            class BatchCompletionCallBack(object):
                ...
                def __call__(se, index):
                    ...
                    if BatchCompletionCallBack.completed[se.parallel] % 100 == 0:
                        ...
                        etl = (end - start) * (self.__len__() / float(BatchCompletionCallBack.completed[se.parallel])) - (
                                end - start)
                        print("Progress : %f %% " %
                              float(BatchCompletionCallBack.completed[
                                        se.parallel] * 100 / self.__len__()) + "ETL %d seconds" % int(etl), end='\r')
