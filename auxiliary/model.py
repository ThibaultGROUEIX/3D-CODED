...
class AE_AtlasNet_Humans(nn.Module):
    def __init__(self, num_points = 6890, bottleneck_size = 1024, nb_primitives = 1, point_translation=False, dim_template=3, patch_deformation=False, dim_out_patch=3):
        ...
        self.count = 0
        self.template = [GetTemplate() for i in range(0,self.nb_primitives)]
        if point_translation:
            if dim_template > 3:
                for i in range(0,self.nb_primitives):
                    self.template[i].vertex = torch.cat([self.template[i].vertex,torch.zeros((self.template[i].vertex.size(0), self.dim_template-3)).cuda()],-1)
        ...
        if patch_deformation:
            ...
            self.templateDiscovery = nn.ModuleList([patchDeformationMLP(patchDim=dim_template, patchDeformDim=dim_out_patch, tanh=True) for i in range(0,self.nb_primitives)])

        self.encoder = PointNetfeat(num_points, bottleneck_size)
        self.decoder = nn.ModuleList([PointGenCon(bottleneck_size = self.dim_before_decoder + self.bottleneck_size) for i in range(0,self.nb_primitives)])
    ...
    def morph_points(self,  x, idx = None):
        ...
        for i in range(0,self.nb_primitives):
            ...
            if not idx is None:
                ...
                rand_grid = rand_grid.view(x.size(0), -1, self.dim_template).transpose(1,2).contiguous() # batch , 2500, 3 -> batch, 6980, 3
            else:
                rand_grid = rand_grid.transpose(0,1).contiguous().unsqueeze(0).expand(x.size(0), self.dim_template, -1) #3, 6980 -> 1,3,6980 -> batch, 3, 6980
    ...
    def decode_full(self, x):
        ...
        for i in range(div-1):
            rand_grid = self.template[0].vertex_HR[batch*i:batch*(i+1)].view(x.size(0), batch, self.dim_template).transpose(1,2).contiguous()
            if self.patch_deformation:
                rand_grid = self.templateDiscovery[i](rand_grid)
            y = x.unsqueeze(2).expand(x.size(0),x.size(1), rand_grid.size(2)).contiguous()
            y = torch.cat( (rand_grid, y), 1).contiguous()
    ...
    def get_patch_deformation_template(self):
        templates = []
        for i in range(0,self.nb_primitives):
            rand_grid = self.template[i].vertex.transpose(0,1).contiguous().unsqueeze(0).expand(1, self.dim_template, -1)
        ...
        if self.point_translation:
            ...
            if self.dim_template==3:
                for i in range(0,self.nb_primitives):
                    ...
                    plt.grid(b=None)
                    plt.axis('off')
                    fig.savefig(os.path.join(path,"points_" + str(i) + "_" + str(self.count)), bbox_inches='tight', pad_inches=0)
            else:
        ...
        if self.patch_deformation:
            ...
            if self.dim_template==3:
                for i in range(0,self.nb_primitives):
                    ...
                    plt.grid(b=None)
                    plt.axis('off')
                    fig.savefig(os.path.join(path,"deformation_" + str(i) + "_" + str(self.count)), bbox_inches='tight', pad_inches=0)
            else:
                print("can't save png if dim template is not 3!")
            self.count += 1
